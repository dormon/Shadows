...
void createPointCloud(vars::Vars&vars){
  ...
  auto color = vars.reCreate<Texture>("gBufferAsPointCloudColor",GL_TEXTURE_2D,GL_RGBA16UI,1,window->x,window->y);
...
void saveGBufferAsPointCloud(vars::Vars&vars){
  ...
  auto pointCouldColor = vars.get<Texture>("gBufferAsPointCloudColor");
