...
const char* gsSourceVisualize = R"(
#version 430 core
#define SHIFT_TRIANGLE_TO_INFINITY
...
uniform bool drawFrontCap = true;
uniform bool drawBackCap = true;
uniform bool drawLightFace = true;
uniform bool drawLightBackFace = true;
...
void main()
{
   ...
   */
    ...
    if(Multiplicity>0 && drawLightFace)
    {
        if(drawFrontCap)
        {
            gl_Position=(mvp*gl_in[FCLF0].gl_Position).SWIZZLE;EmitVertex(); 
		...
		//*
        if(LightPosition.w>0 && drawBackCap)
        {
	...
	if(Multiplicity<=0 && drawLightBackFace)
	{
		if(drawFrontCap)
        {
			gl_Position=(mvp*gl_in[FCLB0].gl_Position).SWIZZLE;EmitVertex(); 
...
const char* fsSourceVisualize = R"(
#version 430 core
