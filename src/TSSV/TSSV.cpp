...
void TSSV::drawSides(glm::vec4 const& lightPosition, glm::mat4 const& viewMatrix, glm::mat4 const& projectionMatrix)
{
	...
	vars.get<VertexArray>("tssv.VAO")->bind();

	Program* program = vars.get<Program>("tssv.program");
	program->use();
	...
	vars.get<VertexArray>("tssv.VAO")->unbind();
}
...
{
	FUNCTION_PROLOGUE("tssv", "model", "adjacency");

	Adjacency const* ad = vars.get<Adjacency>("adjacency");

	Buffer* vbo = vars.reCreate<Buffer>("tssv.VBO", sizeof(float) * 4 * (ad->getNofTriangles() * 3 + ad->getNofEdges()));
...
void TSSV::createElementBuffer()
{
	...
	Buffer* ebo = vars.reCreate<Buffer>("tssv.EBO", sizeof(unsigned) * patchVertices * ad->getNofEdges());
...
void TSSV::createVertexArray()
{
	...
	VertexArray* vao = vars.reCreate<VertexArray>("tssv.VAO");

	vao->addAttrib(vars.get<Buffer>("tssv.VBO"), 0, 4, GL_FLOAT);
	vao->addElementBuffer(vars.get<Buffer>("tssv.EBO"));
}

void TSSV::createProgram()
{
	FUNCTION_PROLOGUE("tssv", "maxMultiplicity", "args.tssv.useRefEdge", "args.tssv.cullSides", "args.tssv.useStencilExport");
	...
	TTS.UseReferenceEdge = vars.getBool("args.tssv.useRefEdge");;
	TTS.CullSides = vars.getBool("args.tssv.cullSides");;
	TTS.UseStencilValueExport = vars.getBool("args.tssv.useStencilExport");;
	...
	vars.reCreate<Program>("tssv.program", TSSilVerHull2, TSSilConHull2, TSSilEvaHull2, TSSilFragHull2);
}

void TSSV::createCapsDrawer()
{
	FUNCTION_PROLOGUE("tssv", "model", "maxMultiplicity");

	vars.reCreate<GSCaps>("tssv.capsDrawer", vars);
}
