...
std::shared_ptr<ge::gl::Shader> getDpsvBuildCS(unsigned int wgSize)
{
...
void swapValues(inout vec3 A,inout vec3 B)
{
	vec3 C=A;A=B;B=C;
}
...
{
	bool swapped = false;
	if(bool(greaterVec(A,B)))
	{
		swapped = !swapped;
		swapValues(A,B);
	}
	if(bool(greaterVec(B,C)))
	{
		swapped = !swapped;
		swapValues(B,C);
	}
...
void TOPTREE_mergeShadowVolumeCastByTriangle( in uint i )
{
		...
		vec3 A  = T.a.xyz - light;
		vec3 B  = T.b.xyz - light;
		vec3 C  = T.c.xyz - light;
		// compute the capping plane equation (i.e. the supporting plane of triangle ABC)
		//const vec3 norm = normalize( cross(C-A, B-A) );
		vec4 capping_plane = computePlane(A, B, C); //vec4( norm, -dot(norm,A) );
		if(capping_plane.w < 0)
		{
			swapValues(A, B);
			capping_plane *= -1.f;
		}

		{
			...
			sp1.plane 		= computePlane(A, B, vec3(0));//computeShadowPlane(A, B);
			sp2.plane 		= computePlane(B, C, vec3(0));//computeShadowPlane(B, C);
			sp3.plane 		= computePlane(C, A, vec3(0));//computeShadowPlane(C, A);
