...
void computeAABB(out vec3 minCorner,out vec3 maxCorner,in int level,in uint node,in uint lii);
vec3 loadAABBCenter(in uint level,in uint node,in uint lii);
...
void loadAABB(out vec3 minCorner,out vec3 maxCorner,in uint level,in uint node,in uint lii){
#if MEMORY_OPTIM == 1
  uint w = aabbPointer[nodeLevelOffset[level] + node*WARP + lii + 1];
