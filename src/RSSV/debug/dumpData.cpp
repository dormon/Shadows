...
void dumpTraverseTriangles(vars::Vars&vars){
  bool old0 = vars.getBool("rssv.param.storeTraverseTrianglesStat");
  bool old1 = vars.getBool("rssv.param.performTraverseSilhouettes");
  bool old2 = vars.getBool("rssv.param.performTraverseTriangles"  );

  vars.getBool("rssv.param.storeTraverseTrianglesStat"  ) = true ;
  vars.getBool("rssv.param.performTraverseSilhouettes"  ) = false;
  vars.getBool("rssv.param.performTraverseTriangles"    ) = true ;

  vars.updateTicks("rssv.param.storeTraverseTrianglesStat");
  vars.updateTicks("rssv.param.performTraverseSilhouettes");
  vars.updateTicks("rssv.param.performTraverseTriangles"  );

  traverse(vars);

  vars.getBool("rssv.param.storeTraverseTrianglesStat"  ) = old0;
  vars.getBool("rssv.param.performTraverseSilhouettes"  ) = old1;
  vars.getBool("rssv.param.performTraverseTriangles"    ) = old2;
