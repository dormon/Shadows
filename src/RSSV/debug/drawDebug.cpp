...
void main(){
  ...
  vars.reCreate<Program>("rssv.method.debug.basicProgram",
...
void main(){
  ...
  vars.reCreate<Program>("rssv.method.debug.drawMortonsProgram",
      make_shared<Shader>(GL_VERTEX_SHADER,vs),
...
void drawBasic(vars::Vars&vars){
  ...
  auto prg = vars.get<Program>("rssv.method.debug.basicProgram");
...
void drawMortons(vars::Vars&vars){
  ...
  depth->bind(1);
  auto prg = vars.get<Program>("rssv.method.debug.drawMortonsProgram");
  prg->use();
