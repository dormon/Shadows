...
void createRasterizeProgram(vars::Vars&vars){
  ...
  auto const storeTraverseStat   =  vars.getBool            ("rssv.param.storeTraverseStat" );
  ...
  vars.reCreate<ge::gl::Program>("rssv.method.rasterizeProgram",
...
void createJobCounter(vars::Vars&vars){
  FUNCTION_PROLOGUE("rssv.method");
  vars.reCreate<Buffer>("rssv.method.jobCounter",sizeof(uint32_t));
...
void createDebugTraverseBuffers(vars::Vars&vars){
  ...
  vars.reCreate<Buffer>("rssv.method.debug.traverseBuffer",sizeof(uint32_t)*(1+1024*1024*128));
}

}

void rssv::rasterize(vars::Vars&vars){
  FUNCTION_CALLER();
  createRasterizeProgram(vars);
  createJobCounter(vars);
  createDebugTraverseBuffers(vars);

  auto prg        = vars.get<Program>("rssv.method.rasterizeProgram");
  ...
  auto jobCounter = vars.get<Buffer >("rssv.method.jobCounter"      );
  ...
  if(vars.addOrGetBool("rssv.method.perfCounters.rasterize")){
    perf::printComputeShaderProf([&](){
