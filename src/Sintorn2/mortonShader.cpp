...
uint morton(uvec3 v){
  ...
  const uint shortestAxis  = uint(uint(bitLength[0] == yBits) + uint(bitLength[0] == zBits)*2u); //uint(uint(shortest == yBits) + uint(shortest == zBits)*2u);
  const uint longestAxis   = clamp(uint(uint(bitLength[2]  == yBits) + uint(bitLength[2]  == zBits)*2u),0u,2u);//clamp(uint(uint(longest  == yBits) + uint(longest  == zBits)*2u),0u,2u);
  ...
  const uint bits1used     = bitLength[2] - bits1Count;//longest - bits1Count;
  const uint bits1DstMask  = uint((1u<<(bitTogether[0]*3u + bitTogether[1]*2u + uint(shortestY & longestX)/*- longestZ*/)) -1u);
